...
public class ArchPdf_1 {
    ...
    */
    ...
    private Font fp = new Font(Font.FontFamily.COURIER, 8);
    ...
    public void crear_PDF(Factura fac) throws  IOException, DocumentException, FileNotFoundException{
        ...
        if(this.ruta_destino!=null){
            ...
            Document mipdf = new Document(PageSize.LETTER,10f,-53f,-15f,0f); //-10,-47,1,1 22derecha -33izquierda -1 arriba abajo
...
//agregar elementos.
    ...
    private void crearContenido(PdfPTable tbcontenedor, Factura fac) throws DocumentException{
        ...
        señor.setPaddingLeft(10.0f);
        señor.setColspan(2);
        señor.setBorder(5);
        señor.setBorderColor(BaseColor.WHITE);
        señor.setVerticalAlignment(Element.ALIGN_TOP);
        PdfPCell nit   = new PdfPCell(new Phrase(fac.getNit(),fuenteNN));
        nit.setBorder(9);
        nit.setPaddingTop(3.0f);
        ...
        PdfPCell direccion = new PdfPCell(new Phrase(String.format("%34s",fac.getDireccion())+"\n",fuenteNN));
        direccion.setColspan(2);
        direccion.setPaddingTop(6.0f);
        direccion.setPaddingBottom(6.0f);
        direccion.setPaddingLeft(15.0f);
        ...
        telefono.setPaddingBottom(6.0f);
...
//
        ...
        PdfPCell valor = new PdfPCell(new Phrase("VALOR",fuenteBoltW)); //fuenteBoltW
...
//            canIte.setPhrase(tx);
            for(int z=0;z<fac.getValores().size();z++){
            PdfPCell canIte = valore.get(z+1);
            //Phrase tx = new Phrase("$ "+darFormatoDec(String.valueOf(fac.getTotal())),fuenteBoltS);
            double total_trabajado = fac.getValores().get(z)* Double.parseDouble(fac.getCantiad().get(z));
            Phrase tx = new Phrase("$ "+darFormatoDec(String.valueOf(total_trabajado)));
            canIte.setPhrase(tx);
            canIte.setHorizontalAlignment(Element.ALIGN_RIGHT);
        }
    ...
    private void crearValores(PdfPTable tbvalores, Factura fac) throws DocumentException {
     float[]medidas = {100.0f,100.0f,100.0f};
        ...
        PdfPCell son;
        son = new PdfPCell(new Phrase(String.format(""+"%5s",fac.getValorTxt()),fuenteNN));
        if(fac.getConceptos().size() == 9){
            son.setPaddingTop(19.0f);
        }
        else{
            son.setPaddingTop(34.0f);
        }
        son.setColspan(2);
        son.setRowspan(3);
        ...
        PdfPCell valSubtotal;
        valSubtotal = new PdfPCell(new Phrase(String.format("  "+"%5s",darFormatoDec(String.valueOf(fac.getSubtotal()))),fuenteNN));
        if(fac.getConceptos().size() == 9){
           valSubtotal.setPaddingTop(19.0f);
        }
        else{
            valSubtotal.setPaddingTop(34.0f);
        }
